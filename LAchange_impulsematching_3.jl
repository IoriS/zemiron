function LAchange_impulsematching_3(payoff_matrix,alpha,beta)
    #alphaは危機回避度上限、betaは下限（元の利得重視）
    #最初は危機回避的、だんだんリスクをおかし、最終的に危機回避的になる人
    p_U = 0
    q_L = 0
    R_1of1 = zeros(4)
    R_1of2 = zeros(4)
    ...
    for i in 1:200
        ...
        newp1 = zeros(4)
        LA_param_list = collect(linspace(alpha,beta,100))
        if i <= 100
            LA_param = LA_param_list[100-i+1]
        else
            LA_param = LA_param_list[i-100]
        end
        for j in 1:4
            if payoff_matrix[j][1] <= s1
                newp1[j] = payoff_matrix[j][1]
            else
                newp1[j] = (LA_param-1)*(payoff_matrix[j][1]+s1)/LA_param
            end
        end
        ...
        for area2 in 1:4
            if payoff_matrix[area2][2] <= s2
                ...
                newp2[area2] = (LA_param-1)*(payoff_matrix[area2][2]+s2)/LA_param
            end
        end
        AA = (newp1[1],newp2[1])
        AB = (newp1[3],newp2[3])
